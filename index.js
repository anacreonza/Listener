...
async function downloadArticle(metaData){
    ...
    try {
        ...
        fs.writeFile(articleFile, JSON.stringify(objectInfo, null, 4), err => {
            if (err) {
                log(`Saving of JSON file failed: ${err}`);
            }
        });
        ...
        fs.writeFile(textFile, contentMetaData.PlainContent, err => {
            if (err) {
                log(`Saving of text file failed: ${err}`);
            }
        })
        // Download original
        if (objectInfo[0].Files[0].FileUrl == null){
            let error = {
                message: "Invalid fileUrl metadata from Woodwing"
            }
            throw error;
        }
        ...
        log(`Download of article failed: ${error.message}`);                  
    }
    ...
    idsHelpers.runXMLExport(iDserverUrl, process.env.INSTANCE, process.env.WWUSERNAME, process.env.WWPASSWORD, basicMetaData.ID, xmlArticleDir, async (result)=>{
        if (result){
            if (result.errorNumber === 0){
                ...
                xmlFiles.map((file)=>{
                    const xmlFile = Path.join(xmlArticleDir, file);
                    log(`Updating XML File: ${xmlFile}`);
                    modifyImageLinksInXML(xmlFile);
                    log(xmlHelpers.convertXMLFileToHTML(xmlFile));
                })
            ...
            log("No return from InDesign Server");
        }
    });
}
